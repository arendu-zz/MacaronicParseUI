...
function remove_all_highlights() {
    _.each(macaronic_sentences, function (sentence) {
        sentence.remove_word_highlights();
    });
}

function make_next_sentence_active(_id, _next_id) {
    _.each(macaronic_sentences, function (ms) {
        console.log(ms.id, _id, _next_id);
        if(ms.id == _id){
            ms.make_nonactive();
        }
        if (ms.id == _next_id){
...
function Node() {
    ...
    this.remove_word_highlight = function () {
        var _view = self.get_view();
        $(_view.textSpan).removeClass('wordhighlight')
    }
    ...
    this.delayed_preview = function () {
        if (self.isMouseOver) {
            self.preview_action()
            self.highlight_word_usage()
        } else {
            //console.log("too late")
        }
    };

    this.highlight_word_usage = function () {
        //console.log("trying to highlight word usage", self.s)
        _.each(macaronic_sentences, function (sentence) {
            _.each(sentence.visible_nodes, function (vn) {
                if (vn.s.toLocaleLowerCase() == self.s.toLocaleLowerCase() && vn != self) {
                    //console.log("found the same word!!", self.s, vn.s)
                    var _view = vn.get_view()
                    $(_view.textSpan).addClass('wordhighlight');
                }
    ...
    this.isTranslationSame = function (modifiedNodes) {
        //TODO: function works, disabled temporarily
        /*var addStr = []
        var removeStr = []
    ...
    this.get_translate_preview_view = function (addNodes, bounds, direction, is_automated) {
        ...
        var trucate_addNodes = addNodes.slice(0,1);
        _.each(trucate_addNodes, function (an) {
            //var ps = document.createElement('div');
            var ps1 = document.createElement('span');
            var ps2 = document.createElement('span');
            ps1.innerHTML = "T";
            ps2.innerHTML = "G";
            $(ps1).addClass("preview translation");
            $(ps1).addClass(an.lang);
            //$(ps1).addClass(direction + "Position");
            $(ps2).addClass("preview translation");
            $(ps2).addClass(an.lang);
            //$(ps2).addClass(direction + "Position");
            if (is_automated) {
                $(ps1).addClass('automated')
                $(ps2).addClass('automated')
            }
            //ps.innerHTML = "here" //an.s;
            //$(ps).append($(ps1))
            //$(ps).append($(ps2))
            //pv.appendChild(ps)
            preview_elements.main.push(ps1)
            preview_elements.main.push(ps2)
        });
...
function MacaronicSentence() {
    ...
    this.initialize = function (mainview) {
        ...
        mainview.append($(self.outer_container));
    ...
    this.remove_word_highlights = function () {
        _.each(self.visible_nodes, function (vn) {
            vn.remove_word_highlight();
        });
    };
